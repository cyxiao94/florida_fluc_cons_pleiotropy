...
freqf <- af(sync_file, gen = c(0,160,360), repl = 1:5)
colnames(freqf)<-c("Anc_F0.R1.freq","Cons_F160.R1.freq","Fluc_F160.R1.freq",
                   "Anc_F0.R2.freq","Cons_F160.R2.freq","Fluc_F160.R2.freq",
                   "Anc_F0.R3.freq","Cons_F160.R3.freq","Fluc_F160.R3.freq",
                   "Anc_F0.R4.freq","Cons_F160.R4.freq","Fluc_F160.R4.freq",
                   "Anc_F0.R5.freq","Cons_F160.R5.freq","Fluc_F160.R5.freq")
covf <- coverage(sync_file, gen = c(0,160,360), repl = 1:5)
colnames(covf)<-c("Anc_F0.R1.cov","Cons_F160.R1.cov","Fluc_F160.R1.cov",
                  "Anc_F0.R2.cov","Cons_F160.R2.cov","Fluc_F160.R2.cov",
                  "Anc_F0.R3.cov","Cons_F160.R3.cov","Fluc_F160.R3.cov",
                  "Anc_F0.R4.cov","Cons_F160.R4.cov","Fluc_F160.R4.cov",
                  "Anc_F0.R5.cov","Cons_F160.R5.cov","Fluc_F160.R5.cov")
...
saveRDS(freqf, file = "Anc_ConsF160_FlucF160_AF.rds")
saveRDS(covf, file = "Anc_ConsF160_FlucF160_COV.rds")
...
anc_freqf_auto <- freqf[!grepl("^X",loci), anc_cols]
anc_cov_auto <- covf[!grepl("^X",loci), anc_cols]
anc_freqf_x <- freqf[grepl("^X",loci), anc_cols]
anc_cov_x <- covf[grepl("^X",loci), anc_cols]

evo_freqf_auto <- freqf[!grepl("^X",loci), evo_cols]
evo_cov_auto <- covf[!grepl("^X",loci), evo_cols]
evo_freqf_x <- freqf[grepl("^X",loci), evo_cols]
evo_cov_x <- covf[grepl("^X",loci), evo_cols]
...
#2. permutation
```{r}
#run permutation_cmh.R on the cluster!!!!
#Follow the suggestion from Reviewer2
#the allele frequency of each SNP will be randomly shuffled among evolved replicates

library(optparse)

option_list <- list(
  make_option(c("-i", "--permu"), type = "integer")
)
...
fwrite(dt_sub, 
       file = paste0("Permutation", permu, "_CMH_result.txt.gz"),
       quote = FALSE, sep="\t")
...
##permutation result
```{r}
perm_result <- fread("summary_permutation_cmh_test.txt")

#p-value for the proportion of SNP shared
p_value <- (sum(perm_result$Permutation<0.064)+1)/1001

#p-value for the allele frequency change differences
