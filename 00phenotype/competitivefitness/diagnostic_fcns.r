  ...
  if(class(x)[[1]]=="glmerMod"){
    ...
    n.model.terms=n.model.terms+length(summary(x)$theta)
		...
		if(x$modelInfo$family$family=="poisson"){
			...
			sigma.f=exp(as.vector(model.matrix(object=x$call$dispformula, data=x$frame)%*%log(sigma(x))))
			if(x$modelInfo$family$family=="nbinom2"){
				...
				xx=gamma.param.transf(mean=fitted(x), var=NULL, shape=1/x.disp, scale=NULL)
				fitted.var = xx$var
