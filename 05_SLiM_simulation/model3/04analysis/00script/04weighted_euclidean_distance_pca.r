...
for (num_snp in c("10000")){
  for (pleio in c("nopleio")){
    for (set in seq(1,100)){
      pca_loading=readRDS(paste0("plot_pca_new/",num_snp,"targets_",pleio,"_0.001lab_set",set,"_loadings.rds"))
      pca_var=readRDS(paste0("plot_pca_new/",num_snp,"targets_",pleio,"_0.001lab_set",set,"_var.rds"))
      pca_var=pca_var/100
