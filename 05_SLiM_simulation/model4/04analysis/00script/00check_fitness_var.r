...
cal_sd=function(num_snp,ispleiotropy,set,pop){
    #filename1="../03results/03results_10000SNP_pleio_set1_p1.pheno"
    filename1=paste0("../03results/03results_",num_snp,"SNP_",ispleiotropy,"_set",set,"_",pop,".pheno")
    f1=fread(filename1)
    ...
    merged_dt$percent_lab="pleiotropy"
    merged_dt$set=set
    merged_dt$pop=pop
...
num_snp=10000
for (ispleiotropy in c('pleio')){
    for (set in seq(1,100)){
        for (pop in c(paste0("p",seq(1,10)))){
            temp_f=cal_sd(num_snp,ispleiotropy,set,pop)
